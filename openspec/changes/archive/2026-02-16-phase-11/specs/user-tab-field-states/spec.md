...
- **Secondary line** (font-size 12px, below label): shown when `copiedValue` is non-empty OR `message` is non-null
...
| `paste_failed` | `✕` | `FAILED` | color #FFFFFF on bg #DC2626 | copiedValue (if any) + message in #DC2626 |
| `skipped` | `⊘` | `SKIPPED` | color #D97706 on bg #FEF3C7 | copiedValue (if any) + message in #D97706 |
...
#### Scenario: SKIPPED row renders reason message
- **WHEN** a field row has `state: "skipped"` and `message: "Field already has value"`
- **THEN** the row SHALL show the `⊘` icon, `SKIPPED` badge (amber), and `"Field already has value"` on the secondary line
